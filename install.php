...
try {
    ...
    echo json_encode(['error' => 'Database connection failed. Link MySQL variables on the backend service.']);
    exit;
}
...
if (tableExists($db, 'users') && tableExists($db, 'members')) {
    echo json_encode(['ok' => true, 'message' => 'Already installed. You can log in with admin / admin123.']);
    exit;
}
...
$steps = [
    ...
    'CREATE TABLE IF NOT EXISTS payments (
      id INT AUTO_INCREMENT PRIMARY KEY,
      txn_code VARCHAR(20) NOT NULL UNIQUE,
...
try {
    ...
    $ins->execute(['admin', $adminHash, 'Admin User', 'superadmin']);
    $ins->execute(['staff', $staffHash, 'Staff Member', 'staff']);

    $db->exec("INSERT IGNORE INTO members (member_code, full_name, phone, plan, status, checkins, joined_at) VALUES
    ...
    $db->exec("INSERT IGNORE INTO payments (txn_code, member_id, plan, amount, method, status, paid_at) VALUES
    ...
    $db->exec("INSERT IGNORE INTO equipment (eq_code, name, category, quantity, condition_, status, location) VALUES
    ...
    $db->exec("INSERT IGNORE INTO products (prd_code, name, category, price, stock, description) VALUES
    ...
    echo json_encode([
        'ok' => true,
        'message' => 'Database ready. Log in with admin / admin123',
    ]);
} catch (Throwable $e) {
    http_response_code(500);
